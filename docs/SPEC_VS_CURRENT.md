...
| Scorecard sharing | Shareable card image, social-ready | ✅ Built — `ScorecardImage` renders to PNG via `react-native-view-shot`; `CardPresentation` is a runway-style trophy moment (card centered at native 9:16 with cyan/yellow halo + 4-corner MoneyParticles burst on reveal, ChevronRunway pulsing upward to a glowing cyan SHARE button). Two-path share: tap SHARE / swipe-up → OS share sheet with the cached PNG (iOS uses RN `Share.share`; Android uses `expo-sharing` since RN's `url` field is iOS-only). Privacy/screenshot behavior: iOS renders a full clean card under the full presentation and hides the presentation layer from capture via a scoped native secure overlay; AppState swap still protects app-switcher/control-center snapshots. Android uses `expo-screen-capture`'s post-capture screenshot listener to auto-open the share sheet with the clean PNG (no Android pre-capture hook exists at the OS level). Filename `Those-I-FCKd-April-11-26.png` for the share receiver. |
