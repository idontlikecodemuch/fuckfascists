...
**`'wont'` rendered-scorecard typo — RESOLVED in same session, follow-up commit:**

The U+2019 curly apostrophe was being dropped during react-native-view-shot's JPEG capture of the rendered scorecard (likely a combination of the curved-thin glyph getting anti-aliased away by JPG q=0.88 compression against the starfield background, plus possibly Fabric/view-shot text-handling quirks). Source code looked correct but the rendered output read `wont` not `won't`. Fix shipped: swap `won’t` → `won't` (straight ASCII apostrophe, U+0027) in `sharedCopy.brandTagline` and `brandTaglineStacked`. The straight apostrophe renders as a tall thin vertical line that survives both font-render quirks and JPG compression. Same character semantically — readers don't perceive the difference at body-text size. Affects 4 surfaces (rendered scorecard, info screen about plaque, launch screen messages, onboarding tagline) — same straight apostrophe used uniformly for cross-surface consistency. Verified visually on device by creator. Comment added to `sharedCopy` block to prevent a future contributor from "fixing" it back to a curly apostrophe and regressing.
