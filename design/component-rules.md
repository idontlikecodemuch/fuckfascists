...
**Rendering note:** The map card host stays mounted after first open. Visibility is controlled by the parent via opacity / pointer-events / accessibility gating instead of unmounting the card subtree. This is intentional: `SpriteView` uses an `overflow: 'hidden'` crop box around a larger sprite-sheet image, and RN 0.76 + Fabric can corrupt clipped native geometry after remounts or rerenders. The sprite perch and crop box must stay fixed-size and `collapsable={false}`, and the sheet must move via transforms rather than negative layout offsets. While hidden, the sprite image is parked far outside the crop box so switching from a `2x1` to a `2x2` sheet, or vice versa, does not flash the old crop region for a frame on reopen. For v1.5, the planned permanent escape hatch is pre-sliced per-frame card sprite PNGs so the card no longer depends on runtime sheet cropping.
