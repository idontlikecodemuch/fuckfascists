 ...
 *
 * Why we care:
 *   - iOS fires `inactive` synchronously when a screenshot is initiated, when
 *     the user pulls down Control Center, when the App Switcher previews the
 *     app, or when an interruption (incoming call) covers the screen. A JS
 *     swap to a clean PNG-only view during `inactive` means the captured
 *     screenshot and App Switcher snapshot are the rendered card alone — no
 *     starfield, no chrome.
