...
- **Stable array/object deps — use the sync-ref pattern** — when a hook or component receives an array or object prop that is stable in practice but not guaranteed to have a stable reference (e.g. loaded once in App.tsx but passed down through several components), use refs to decouple it from `useEffect`/`useCallback` deps: `const ref = useRef(val); ref.current = val;` (two lines, no dep array, runs synchronously every render). The effect/callback then reads `ref.current` instead of the prop directly. This prevents spurious effect re-runs without introducing staleness. Used in `useScorecard` (entities, platforms → prevents unnecessary `aggregateScorecard` DB re-reads) and `useBarcodeSearch` (entities → stable `resolveBarcode` callback).
- **Infinite render loop — unmemoized hook return values as `useEffect` dependencies** — if a hook computes an array or object inline in its body (not wrapped in `useMemo`), it produces a new reference on every render. Using that as a `useEffect` dependency causes the effect to fire every render → state update → render → repeat. In Fabric (new arch) this combines lethally with `LayoutAnimation`: rapid successive state updates during a layout animation commit cause the C++ reconciler's internal view-list sort to receive inconsistent data → `SIGABRT` with `_LIBCPP_ASSERT_SEMANTIC_REQUIREMENT: "Your comparator is not a valid strict-weak ordering"`. **Rules to prevent recurrence:** (1) Any array or object returned from a hook must be wrapped in `useMemo` with stable deps. (2) Data derived entirely from other state/props should use `useMemo`, not `useState + useEffect`. (3) Never use an unmemoized hook return value (e.g. `hook().items`) as a `useEffect` or `useCallback` dependency — the reference will change every render. This occurred in `usePlatformAvoidance` (`items` computed inline) + `TrackContext` (`todayActions` via `useState + useEffect([avoidance.items])`). Fixed by memoizing `items` in the hook and converting `todayActions` to `useMemo`.
