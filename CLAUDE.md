...
**Landed 2026-04-27, hardened 2026-04-28.** On physical devices, a business card sprite could render correctly on first open, then come back cropped to the top of the head after dismissing and reopening the same or another card. The failure pattern looked like layout at first glance, but the actual trigger was renderer lifecycle: the card subtree was being unmounted on dismiss, and RN 0.76 + Fabric could recycle the clipped native view that `SpriteView` uses for sprite-sheet cropping with stale geometry. Upstream Expo 52 / RN 0.76 reports also show `overflow: hidden` corruption after rerenders, so persistent mount is only one layer of the fix.

**Fix:** `MapScreen.tsx` keeps the full-card host mounted after first open and toggles visibility with opacity, pointer events, and accessibility flags rather than conditional unmounting. `BusinessCard.tsx` resets transient animated state (`translateY`, `shakeX`, `shakeY`) when the hidden card becomes visible again, and the sprite perch is fixed-size + `collapsable={false}`. `SpriteView` keeps the crop box `collapsable={false}`, moves the sheet via transforms instead of negative layout offsets, and parks the bitmap far above the crop box while hidden so sheet changes do not flash the wrong crop region. `ios/Podfile` also backports the `RCTViewComponentView` stale layoutMetrics recycle fix during `pod install`.
...
- For v1.5, the durable fallback is to pre-slice card sprite sheets into per-frame PNGs and stop relying on runtime `overflow: 'hidden'` cropping for the card sprite. Current estimate: reachable frames add ~8 MB raw PNG; all manifest frames add ~16 MB.
