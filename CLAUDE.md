...
- Medium-term public-review improvement: split the committed source-of-truth people file into simple alphabet shards, then assemble the monolith for existing scripts. Preferred first pass: `assets/data/people/meta.json`, `assets/data/people/people-a-m.json`, `assets/data/people/people-n-z.json`, plus an `npm run assemble:people` script that validates duplicate IDs, confirms `_meta.totalPeople`, sorts deterministically by `id`, and writes generated `assets/data/people.json`. Keep `assets/data/people.bundle.json` as the runtime payload. This keeps review easy without hiding changes behind Git LFS.
