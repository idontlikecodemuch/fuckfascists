...
├── modules/
│   └── mapkit-search/               ← local Expo native module (iOS only)
│       ├── package.json             ← name: "mapkit-search"; registered in root package.json as "file:./modules/mapkit-search"
│       ├── expo-module.config.json  ← declares MapKitSearchModule for apple platform; drives CocoaPods autolinking
│       └── ios/
│           ├── MapKitSearch.podspec ← depends on ExpoModulesCore; source_files: **/*.{h,m,swift}
│           └── MapKitSearchModule.swift ← MKLocalPointsOfInterestRequest bridge; auto-linked on prebuild
└── assets/
